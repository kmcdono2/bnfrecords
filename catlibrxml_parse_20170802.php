...
//		$max = 1000;
		...
		$url .= "(bib.anywhere%20all%20%22Catlibr%22)%20and%20(bib.publicationdate%20%3E=%201700)";
		$url .= "%20and%20";
		$url .= "(bib.publicationdate%20%3C=1799)";
										...
										if(($srw_recordData_value["tag"] == "srw_recordData")) {
											foreach($srw_recordData_value as $mxc_records_key => $mxc_records_value) {
												...
												//	print "ID"." : ".$mxc_record["attributes"]["id"]."\n";
													...
													$query = "SELECT ID, UUID FROM Catlibr_XML_UUID WHERE ARK = \"$ID\"";
													$mysqli_result = mysqli_query($mysqli_link, $query);
